...
def sync_action_project(project_config):
    ...
    for artifact in action_info.get('artifacts', []):
        ...
        if should_download_asset(artifact_name, asset_patterns):
            print(f"  正在下载Artifact: {artifact_name}")
            save_path = os.path.join(target_dir, f"{artifact_name}.zip")
            
            # GitHub Artifact下载URL
            download_url = artifact['archive_download_url']
            
            if download_file(download_url, save_path):
                downloaded_count += 1
                size_mb = artifact['size_in_bytes'] / 1024 / 1024 if artifact['size_in_bytes'] else 0
